...
All third-party actions pinned to commit SHAs (see workflow files). **`F18` ✅ Closed 2026-09-12:** all 5 workflows now declare `permissions: contents: read` at the top level (previously none did, so each inherited the default, possibly read-write, `GITHUB_TOKEN` scope).
...
| M2 | 🔶 In progress | Library & Reading UI (SwiftUI macOS) — import + RSVP wired; library/theme/flow-view not started; nothing compiler-verified yet. Security gate before exit: `F14`–`F23` ✅ closed in code 2026-09-12; `F24` remains open — its code-side prep is done, but it has no effect until this repo is hosted on GitHub and Dependabot is enabled there (see its register row). |
| M3 | — | RSVP view, annotations, accessibility pass. Security gate: `F20`/`F21` ✅ closed 2026-09-12; `F25` (apple-build CI pipeline) still open, `A7` (ADR-009 addendum) written before OCR wiring begins, entitlements review (`A3`) done. |
