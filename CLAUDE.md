...
2. Library grid/list view (collections, tags, sort/filter, FTS search) — **not started.** `LibraryView.swift` is a flat, unsorted `List`; `SidebarView.swift` is an 11-line placeholder. No collections/tags data model exists anywhere (`gist-model`/`gist-store`). FTS search now has a Rust-side path (`GistCore::search_items` in `gist-ffi`, see `search_items` note above), but `apps/apple/Generated/gist_ffi.swift` hasn't been regenerated since (still dated 2026-09-10, no `search_items`/`searchItems` symbol in it — `./tools/gen-bindings.sh` needs a run before Swift can even see the new export), and there's no UI entry point regardless — `LibraryView` has no search field and `CoreClient` has no wrapper method for it.
