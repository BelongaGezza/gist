...
Exit criterion: a team member can use it as their daily reader. Not met yet — a debug build now compiles and launches (see Current state above), but the theme/flow-view gaps above are real blockers, and the app has never been run through its actual UI by a person, so day-to-day usability is unverified. `xcodebuild build`/`test` now exercises 11 real `CoreClient`/`LibraryFiltering` tests (not just a placeholder — see M2 item 2), but that's compiler+logic verification, not a substitute for a person driving the actual UI.
...
| M2 | 🔶 In progress | Library & Reading UI (SwiftUI macOS) — import + RSVP + search/removal/URL-import/collections wired, compiler-verified and now unit-tested (`xcodebuild test` passes 11 real `CoreClient`/`LibraryFiltering` tests against a real temp-dir `GistCore`, 2026-09-12); theme/flow-view not started, no XCUITest/full-UI coverage yet. Security gate before exit: `F14`–`F23` ✅ closed in code 2026-09-12; `F24` remains open — its code-side prep is done, but it has no effect until this repo is hosted on GitHub and Dependabot is enabled there (see its register row). |
