...
| M2 | 🔶 In progress | Library & Reading UI (SwiftUI macOS) — import + RSVP wired; library/theme/flow-view not started; nothing compiler-verified yet. Security gate before exit: `F14`–`F24` ✅ all closed in code 2026-09-12 (`F24` still needs the Dependabot GitHub-Settings toggle flipped by a human — see its register row). |
...
| F24 | Low | ✅ Closed 2026-09-12 (partial — see note) | `cargo deny check advisories` is already documented above as non-functional in this dev environment, and there was no compensating control at all. Fixed the code-side half: `.github/dependabot.yml` now configures weekly `cargo` and `github-actions` version-update PRs. **Still needs a human to flip a toggle:** GitHub's "Dependabot alerts" and "Dependabot security updates" (Settings → Security) are account/repo-level settings, not something a config file can enable — do that before treating this as fully closed. |
