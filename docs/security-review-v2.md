...
**Update 2026-09-24 (documentation-accuracy correction):** both gaps described in the paragraph above have since been closed — the paragraph is left as history (this pass's honest finding as of 2026-09-18) rather than rewritten, per this project's documentation-accuracy convention (see `CLAUDE.md`). `A3` was closed the same day as this review, later in the session: `apps/apple/macOS/GISTmacOS.entitlements` was added (app-sandbox, user-selected file read-write, network client only) and verified embedded via `codesign -d --entitlements -` on an ad-hoc-signed build (ADR-012). `A6` was also closed the same day: AES-256-GCM encryption-at-rest was decided (ADR-011), Rust-side implemented, and a real Keychain round trip verified — then, later the same day, ADR-014 (per-item on-demand encryption) switched it on for production, so `CoreClient.shared` can now genuinely read an item a user explicitly encrypts (new imports still default to plaintext). This session's own four new findings (`N1`–`N5` below) were also all fixed later the same day. See the per-item update notes added throughout this document, and `CLAUDE.md`'s `A3`/`A6`/`N1`–`N5` security-register rows for the authoritative closure detail — not reproduced in full here to avoid duplicating that log.
...
| `F24` (no GitHub remote) | Dependabot config inert | `git remote -v` returns nothing; confirmed. **Update 2026-09-24: no longer accurate — closed 2026-09-19.** The repo is now hosted at `https://github.com/BelongaGezza/gist`, and Dependabot alerts/automated security fixes were verified enabled via the GitHub API. See `CLAUDE.md`'s `F24` row. |
| `A3` (entitlements) | Still fully open | No `.entitlements` file anywhere in the repo; `apps/apple/macOS/Info.plist` has no sandbox/ATS/hardened-runtime keys at all. **Update 2026-09-24: no longer accurate — closed 2026-09-18** (the same day this review was written, later in the session). `apps/apple/macOS/GISTmacOS.entitlements` was added and verified embedded via `codesign -d --entitlements -` (ADR-012). One residual caveat: no person has yet done a runtime click-through of a sandboxed launch. See `CLAUDE.md`'s `A3` row. |
| `A6` (encryption at rest) | Still fully open | `Store::insert_item` (`crates/gist-store/src/lib.rs:211-218`) and `store_original_copy` (`:311-313`) both write plaintext via `std::fs::write` with no encryption layer. **Update 2026-09-24: no longer accurate — decided, implemented and switched on for production 2026-09-18** (the same day this review was written, later in the session), via ADR-011 + ADR-014. New imports still default to plaintext; only an item a user explicitly encrypts is affected, and it is now genuinely readable afterward through `CoreClient.shared`. See `CLAUDE.md`'s `A6` row for the full detail. |
...
**Update 2026-09-24: ✅ Closed 2026-09-18** (the same day this review was written, later in the session). Both fixes above were applied: `fuzz/Cargo.toml` gained a `[workspace]` table, and all four targets now build `ParseLimits` via `..ParseLimits::default()`. `cd fuzz && cargo check` is genuinely clean. The residual gap (cargo-fuzz not installed, targets never actually run) was closed the same session too: `cargo-fuzz` 0.13.2 and a `nightly` toolchain were installed (scoped to `fuzz/` invocations only), and all four targets were actually run at `-max_total_time=100` each — zero crashes, zero ASAN failures across ~7M combined executions. This is a first real pass, not exhaustive fuzzing. See `CLAUDE.md`'s `N1` register row.
...
**Update 2026-09-24: ✅ Closed 2026-09-18** (same session as this review). `fuzz_web_extract.rs` was rewritten to feed fuzzer bytes directly into `gist_web::extract_content` as HTML instead of treating them as a URL. Actually run via `cargo +nightly fuzz run fuzz_web_extract -- -max_total_time=100`: 1,460,475 execs in ≈101s, zero crashes. See `CLAUDE.md`'s `N2` register row.
...
**Update 2026-09-24: ✅ Closed 2026-09-18** (same session as this review). `gist-parse-docx::read_zip_entry_limited` now threads one cumulative `total_expanded` accumulator across the `styles.xml`/`numbering.xml`/`document.xml` reads, matching `gist-parse-epub::parse_spine`'s shared-budget pattern, per the first recommendation above. See `CLAUDE.md`'s `N3` register row.
...
**Update 2026-09-24: ✅ Closed 2026-09-18** (same session as this review). `prepare_image` now calls `ImageReader::into_dimensions()` to peek declared width/height and reject an oversized image before the full decode, with the post-decode pixel-count check retained as defence in depth. Still dormant/no live caller (`import_image_with_ocr` is `todo!()` until M3) — this closes the finding but the code path itself isn't exercised by a real caller yet, and the broader `A7` item (ADR-009 addendum + OCR wiring) remains open, M3 gate. See `CLAUDE.md`'s `N4`/`A7` register rows.
...
**Update 2026-09-24: ✅ Closed 2026-09-18** (same session as this review). `.gitignore`'s stale comment was replaced with one matching actual practice (both root and `fuzz/` lockfiles are intentionally tracked). See `CLAUDE.md`'s `N5` register row.
...
| `F24` | Low | Open — inert until GitHub-hosted, historical row — **✅ Closed 2026-09-19, see update below** | No remote exists; confirmed. **Update 2026-09-24:** the repo is now hosted at `https://github.com/BelongaGezza/gist`; Dependabot alerts/automated security fixes verified enabled via the GitHub API. See `CLAUDE.md`'s `F24` row. |
| `A3` | Architecture | Open — `M3` gate, fully unaddressed, historical row — **✅ Closed 2026-09-18, see update below** | Zero sandbox/entitlements/ATS/hardened-runtime configuration exists yet, not partially done. **Update 2026-09-24:** closed the same day as this review, later in the session — `apps/apple/macOS/GISTmacOS.entitlements` added and verified embedded via `codesign` (ADR-012). One residual caveat: no runtime click-through by a person yet. See `CLAUDE.md`'s `A3` row. |
| `A4`/`A6` | Architecture | Open — `M4`, historical row — **`A6` ✅ decided/implemented/switched on for production 2026-09-18; `A4` still open, see update below** | No at-rest integrity or confidentiality; plaintext confirmed at every storage write site. **Update 2026-09-24:** `A6` (ADR-011 encryption-at-rest, then ADR-014 per-item on-demand encryption) was decided, Rust-side implemented, Keychain-verified, and switched on for production the same day as this review — new imports still default to plaintext, but an explicitly-encrypted item is genuinely readable through `CoreClient.shared`. `A4` (ADR-013 at-rest integrity, BLAKE3 sidecars) was also decided and Rust-side implemented the same day but has no Swift/FFI/UI wiring yet — still meaningfully open, unlike `A6`. See `CLAUDE.md`'s `A4`/`A6` rows. |
| `A7` | Architecture | Open — `M3` gate, historical row — **partially closed 2026-09-18, see update below** | **Sharpened by `N4`**: the exact decode-before-check pattern this item warns about already exists in `gist-imageprep::prepare_image`, dormant only because nothing calls it yet. **Update 2026-09-24:** the concrete code-level gap (`N4`, this same row) was fixed the same day as this review — see `N4`'s row below. The ADR-009 addendum and the OCR wiring itself remain open, unchanged, `M3` gate. |
| **`N1`** | **Medium** | **Open — new, historical row — ✅ Closed 2026-09-18, see update below** | Fuzz harnesses don't compile (missing `ParseLimits` field + missing `[workspace]` table); unrelated to cargo-fuzz's absence from this environment. **Update 2026-09-24:** fixed the same day as this review, later in the session, including actually running all four targets (~7M combined executions, zero crashes) after installing `cargo-fuzz`/nightly. See `CLAUDE.md`'s `N1` row. |
| **`N2`** | **Low** | **Open — new, historical row — ✅ Closed 2026-09-18, see update below** | `fuzz_web_extract` never reaches the HTML-parsing code it's meant to fuzz. **Update 2026-09-24:** rewritten to fuzz `extract_content` directly and actually run (1,460,475 execs, zero crashes) the same day as this review. See `CLAUDE.md`'s `N2` row. |
| **`N3`** | **Low** | **Open — new, historical row — ✅ Closed 2026-09-18, see update below** | DOCX's `max_expanded_bytes` is per-part (~3× effective budget), unlike ePub's cumulative enforcement. **Update 2026-09-24:** fixed the same day as this review via a shared accumulator matching ePub's pattern. See `CLAUDE.md`'s `N3` row. |
| **`N4`** | **Medium (dormant)** | **Open — new, feeds into `A7`, historical row — ✅ Closed 2026-09-18, see update below** | `prepare_image` decodes before size-checking; must be fixed before `M3` OCR wiring, not just documented as a future ADR addendum. **Update 2026-09-24:** fixed the same day as this review (`ImageReader::into_dimensions()` pre-decode check). Still dormant/no live caller until `M3`'s OCR wiring lands. See `CLAUDE.md`'s `N4` row. |
| **`N5`** | **Informational** | **Open — new, historical row — ✅ Closed 2026-09-18, see update below** | `Cargo.lock` tracked despite `.gitignore` listing it; align the ignore file with actual practice. **Update 2026-09-24:** `.gitignore`'s stale comment corrected the same day as this review. See `CLAUDE.md`'s `N5` row. |
...
**Update 2026-09-24 (documentation-accuracy correction):** all five priority items above (`N1`, `N4`, `A3`, `N3`, `N2`/`N5`) were in fact completed 2026-09-18 — the same day this review was written, later in the same session — not left for "the next work session" as this section originally framed it. `A6` was also decided/implemented/switched-on-for-production the same day, ahead of its `M4` schedule. See the per-finding update notes above and `CLAUDE.md`'s security register for verification detail (file/line references, tests added, commands re-run). Nothing from this review's own findings remains open as of this correction; `F10`, `A4`, and `A7`'s ADR-009-addendum/OCR-wiring half correctly remain scheduled at their milestone gates (`M4`, `M4`, `M3` respectively) and don't need to move.
