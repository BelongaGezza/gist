...
- `SCHEMA_VERSION = 3`. Migration v1→v2: `tokens` shadow table + `fts_index` FTS5 virtual table (`porter unicode61`) + sync triggers. `[A1]` ✅. Migration v2→v3 (2026-09-12): `collections`/`item_collections`/`tags`/`item_tags` tables, all join rows `ON DELETE CASCADE` from `library_items`, with `create_collection`/`list_collections`/`add_item_to_collection`/`remove_item_from_collection`/`list_items_in_collection`/`add_tag`/`remove_tag`/`list_tags_for_item` CRUD — schema/store layer only, no `gist-core`/`gist-ffi` wiring yet (see M2 milestone section).
