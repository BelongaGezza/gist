...
- `[F24]` ✅ **Was Low, fixed 2026-09-12 (partial).** `cargo deny check advisories` is already known non-functional in this dev environment (see §0.3 and CLAUDE.md), and there was no compensating control at all. Fixed the code-side half: `.github/dependabot.yml` now configures weekly `cargo` and `github-actions` version-update PRs (the latter also serves `[F3]`'s SHA-pinning policy — Dependabot bumps the pinned SHA when a new release lands). **Still needs a human step:** GitHub's "Dependabot alerts"/"Dependabot security updates" (Settings → Security) are an account/repo toggle, not something this file can enable — flip it before treating this as fully closed.
...
**Exit criterion:** a team member can use it as their daily reader, including searching their library and removing items they no longer want. Not met yet — no compiled build exists to try, and the library/search/removal/theme/flow-view gaps above are real blockers even once it compiles. `[F14]`–`[F24]` are all fixed in code (2026-09-12) — `[F24]` still needs a human to flip the Dependabot alerts/security-updates toggle in GitHub Settings before it's fully closed, but that's no longer a code blocker.
...
| F24 | Low | ✅ Closed 2026-09-12 (partial) | (audit `L-7`). No Dependabot config. Fixed the code-side half via `.github/dependabot.yml`; still needs "Dependabot alerts"/"security updates" enabled in GitHub Settings by a human. See §1.3. |
...
**Open items blocking release:** F10 (M4), A3 (M3), A4 (M4), A6 (M4). F12 accepted and deferred. F14–F24 all fixed in code 2026-09-12 (see above; F24 still needs a human to flip the Dependabot GitHub-Settings toggle). Additionally open per the 2026-09-12 audit: F25/A7 (before M3).
