...
**Fifth pass, same day (2026-09-12):** all five Low findings fixed and verified (full workspace `cargo test`/`clippy -D warnings`/`fmt --check`/`cargo deny check bans licenses sources` green). `[F20]` — epub `check_drm` now rejects on ambiguity (no `Algorithm` attribute, or any value other than the IDPF exemption, is treated as DRM); 3 new tests. `[F21]` — new `gist-web::robots_url_for` builds the robots.txt URL via `Url::join`, preserving an explicit port instead of always checking 443; 2 new tests. `[F22]` — new `gist-ffi::install_panic_hook_once` (guarded by `std::sync::Once`, called from `ffi_catch!`) routes panic messages through `tracing::debug!` instead of the default stderr write, and is a deliberate no-op under `cfg(test)` so it doesn't silence other tests' panic diagnostics in the same binary; 3 new tests. `[F23]` — new `gist-store::sanitize_ext` reduces a content-addressed copy's file extension to ASCII alphanumeric characters at `store_original_copy`'s own boundary, regardless of caller; 2 new tests including one confirming a traversal-attempt `ext` still resolves inside `originals/`. `[F24]` — `.github/dependabot.yml` added for weekly `cargo` and `github-actions` version updates; **remains open, not partial** — this repo has no GitHub remote at all (see §1.3), so the file is inert today regardless of "Dependabot alerts" settings, which also can't be configured until the repo is actually hosted on GitHub. `[F25]` and `[A6]`/`[A7]` remain open — out of scope for this pass (CI-pipeline and ADR-writing work, not code fixes) per prior instruction not to pre-decide architectural content.

**Sixth pass, documentation-drift correction (2026-09-12):** `[F25]` was actually closed in code the same day as the passes above, and `CLAUDE.md` was updated at the time to say so — but this document was not, and kept describing `apple-build` as nonexistent/open in five separate places (§1.3's table and its `[F25]` note, §5's M3 task list and exit criterion, §6's R15, and §8's register table and closing summary). Found and corrected in this pass, no new code changes: `.github/workflows/apple-build.yml` exists (XcodeGen → `xcodebuild build`/`test` on the `GISTmacOS` scheme, gated on PRs touching `apps/apple/**`, `crates/gist-ffi/**`, or the bindings/xcframework tooling), and both `xcodebuild build` and `xcodebuild test` have actually been run and verified locally against it — catching and fixing a real bug along the way: XcodeGen's autogenerated `GISTmacOS` scheme didn't wire `GISTTests` as testable, and `GISTTests`'s inferred `TEST_HOST` pointed at a nonexistent binary because `GISTmacOS`'s `PRODUCT_NAME` override (`GIST`) differs from its target name — both fixed in `apps/apple/project.yml` (explicit `schemes: GISTmacOS` test-action wiring, plus explicit `TEST_HOST`/`BUNDLE_LOADER`). See `CLAUDE.md`'s CI-pipelines section for the full step breakdown, which was already accurate. Every place below now reads `[F25]` ✅ Closed, matching `CLAUDE.md`. The "prepared but not yet running *in GitHub CI*" caveat still applies, same as the other 5 workflows, since this repo has no GitHub remote (§1.3) — closing `[F25]` means the pipeline was written and its steps verified locally, not that it has executed on GitHub's infrastructure.

**Seventh pass, documentation-drift reconciliation (2026-09-24):** this document had drifted materially behind `CLAUDE.md` and `PLATFORM_VERIFICATION.md` (both actively maintained; this file is not) on five points. Corrected in place at each affected section below, per this document's own established convention (see the sixth-pass note above) — dated update notes appended, nothing rewritten wholesale:
1. **`[F24]`** (§1.3, §5 M2 exit criterion, §8) — ✅ closed 2026-09-19, not open: the repo is now hosted at `https://github.com/BelongaGezza/gist` and Dependabot alerts/automated security fixes were verified enabled via the GitHub API (not merely requested — independently confirmed with a follow-up read), which then immediately proved itself live via ~19 real Dependabot PRs the same week.
2. **`[A3]`** (§3.10, §5 M3, §6 R7, §8) — ✅ closed 2026-09-18, not open: `apps/apple/macOS/GISTmacOS.entitlements` (ADR-012) is verified embedded via `codesign -d --entitlements -` on an ad-hoc-signed build. One residual caveat carries forward: no person has yet done a runtime click-through of an actually-sandboxed launch.
3. **`[A6]`** (§5 M4, §7 Q11, §8) — decided, Rust-side implemented, Keychain round-trip runtime-verified, **and switched on for production** 2026-09-18 via ADR-014 (per-item on-demand encryption, `CoreClient.shared` now reads via `newWithReadKey`/a real `KeychainKeyProvider`). Nuance that matters: new imports still land plaintext by default, unchanged — only an item a user explicitly runs "Encrypt" on is affected. `[A4]` (BLAKE3 integrity, the item this document's Q11/§5 M4 pairs with `A6`) is a separate decision, still only Rust-side implemented and not yet switched on for production (no Swift/UI wiring) — do not conflate the two statuses.
4. **`[A7]`** (§2.7, §3.3, §5 M3, §8) — partially resolved, not open outright: the concrete code-level gap this item warned about (`gist-imageprep::prepare_image`'s decode-before-check ordering, independently found and tracked as `N4` in `docs/security-review-v2.md`) was fixed 2026-09-18. What's still open: the ADR-009 addendum documentation itself, and the OCR wiring (`Core::import_image_with_ocr` is still `todo!()`) — both remain M3 scope.
5. This document has never mentioned the Windows shell work (`apps/windows`, ADR-015–018, workstreams W0–W2) that `PLATFORM_VERIFICATION.md` shows verified 2026-09-20 through 2026-09-22 — see the cross-reference notes added at §1.1 and §6 R11. Full detail is deliberately not duplicated here; `docs/windows-development-plan.md` is the authoritative plan for that work, per its own header.

No disagreement was found between `CLAUDE.md` and `PLATFORM_VERIFICATION.md` on any of the above during this pass — both were read in full and cross-checked before writing these corrections.
...
│   │   └── Tests/
│   └── windows/                # Update 2026-09-24: no longer a placeholder — W0/W1/W2 underway since
│                                #   2026-09-20 (ADR-015–018). See docs/windows-development-plan.md and
│                                #   §6 R11's update note; not detailed in this macOS-focused plan.
...
**No GitHub remote exists for this repository at all right now** (confirmed via `git remote -v` returning nothing — this is a local-only repo). Every pipeline in the table below, plus `.github/dependabot.yml`, is prepared configuration that has never actually executed and provides zero protection today: GitHub Actions and Dependabot both require the repo to be hosted on GitHub before any of it can run. The ✅ marks below mean "the YAML is written and internally consistent," not "this has run in CI" — treat that distinction as load-bearing when reading this table.

**Update 2026-09-24 (documentation-accuracy correction):** the paragraph above is no longer true and is left in place only as history, matching `CLAUDE.md`'s own identical correction (its CI-pipelines section, 2026-09-19). The repo has been hosted at `https://github.com/BelongaGezza/gist` (public, pushed from `main`) since 2026-09-19, and GitHub Actions/Dependabot both now execute for real. Doing so immediately surfaced real bugs no local run ever could: `CLAUDE.md`'s `N6`/`N7` security-register rows document nine CI-plumbing/build bugs (unresolvable SHA pins on `dtolnay/rust-toolchain` and `actions/cache`, a missing `cargo-audit` install step, a Homebrew tap-policy break in the XcodeGen install, a `clippy`/`fmt` toolchain-identity mismatch, an MSRV bump, a Swift type-checker timeout in `LibraryView.swift`, and a bzip2/lzma zip-codec link failure) plus 7 real RUSTSEC advisories that fixing `core-quality`'s own cargo-deny version then surfaced — all found and fixed by 2026-09-19, verified on real GitHub Actions runs, not local commands. See `[F24]`'s updated status below (§1.3 "Open" list and §8) for the closure itself; the full bug-by-bug breakdown is in `CLAUDE.md` and not reproduced here to avoid duplicating that log.
...
| `apple-build` | PR touching `apps/apple/**`, `crates/gist-ffi/**`, or the gen-bindings/xcframework tooling | XcodeGen → `xcodebuild build`/`test` on the `GISTmacOS` scheme (Debug, unsigned) | ✅ `[F25]` closed 2026-09-12 |
...
- `[F25]` ✅ **Informational, closed 2026-09-12.** The `apple-build` row above previously described a pipeline that didn't exist. It now does: `.github/workflows/apple-build.yml` was written and — unlike the note this replaces assumed was still needed — `xcodebuild build`/`test` on the `GISTmacOS` scheme have actually been run and verified locally (this dev environment turns out to have full Xcode 26.6, not just Command Line Tools, correcting an earlier false claim in this document's §3.2 gate note and in `CLAUDE.md`). That run caught and fixed a real bug: XcodeGen's autogenerated scheme didn't wire `GISTTests` as testable, and its inferred `TEST_HOST` pointed at a nonexistent binary because `GISTmacOS`'s `PRODUCT_NAME` differs from its target name — both fixed in `apps/apple/project.yml`. Swift code now has a real, locally-verified CI gate; the "prepared but not yet running in GitHub CI" caveat still applies since this repo has no GitHub remote (see above), same as the other 5 workflows. **Documentation-drift note (2026-09-12, sixth pass):** this bullet, the table row above, and four other places in this document (§5 M3 task list/exit criterion, §6 R15, §8's register and closing summary) kept describing this as open for a time after `CLAUDE.md` had already recorded it closed — corrected in this pass, no code changed.
- `[F24]` **Low, still open.** `cargo deny check advisories` is already known non-functional in this dev environment (see §0.3 and CLAUDE.md), and there is no compensating control at all. `.github/dependabot.yml` now configures weekly `cargo` and `github-actions` version-update PRs (the latter would also serve `[F3]`'s SHA-pinning policy — Dependabot bumps a pinned SHA when a new release lands), but **this repo has no GitHub remote yet** (§1.3) — the file has never run and provides no protection today. The project remains just as blind to new RUSTSEC advisories as before this file existed. Close only once the repo is pushed to GitHub *and* "Dependabot alerts"/"Dependabot security updates" are enabled there.
  - **Update 2026-09-24: `[F24]` is closed, not open.** Both preconditions above were met 2026-09-19 — the repo is pushed to `https://github.com/BelongaGezza/gist`, and "Dependabot alerts"/"Dependabot security updates" were enabled via the GitHub API (`PUT .../vulnerability-alerts` → 204; `PUT .../automated-security-fixes` → `{"enabled":true}`, both independently reconfirmed with a follow-up read). Dependabot proved itself live the same day by opening real update PRs. See `CLAUDE.md`'s `F24` register row for the exact closure language this note mirrors, and §8 below.
...
**Open (audit 2026-09-12):**
- `[F20]` ✅ **Was Low, fixed 2026-09-12.** `check_drm()` treated a malformed-but-valid `EncryptionMethod` with no `Algorithm` attribute as "no DRM." Fixed: now rejects on ambiguity — no `Algorithm`, or any value other than the IDPF exemption, is treated as `DrmProtected`, per ADR-004's intent. 3 new tests (ambiguous case, IDPF exemption, commercial algorithm).
...
**Update 2026-09-24: partially resolved, not fully open.** The concrete instance of this gap — `gist-imageprep::prepare_image` itself decoding untrusted bytes (`image::load_from_memory`) before checking the decoded pixel count, independently found and tracked as `N4` in `docs/security-review-v2.md` — was fixed 2026-09-18: it now peeks declared width/height via `ImageReader::into_dimensions()` and rejects an oversized image before the full decode, with the post-decode pixel-count check kept as defence in depth. Still open: the ADR-009 addendum documenting this as policy, and the OCR wiring itself (`Core::import_image_with_ocr` is still `todo!()`) — both remain M3 scope, per `CLAUDE.md`'s `A7`/`N4` register rows.
...
**Open (audit 2026-09-12):**
- `[F21]` ✅ **Was Low, fixed 2026-09-12.** robots.txt lookup dropped the original URL's port, always checking port 443. Fixed: new `robots_url_for` builds the URL via `Url::join` instead of manual string formatting, preserving an explicit port. 2 new tests.
...
`[F22]` ✅ **Was Low, fixed 2026-09-12.** `ffi_catch!` never leaked panic payloads into `GistError` — correct — but installed no custom `std::panic::set_hook`, so the *default* panic message still hit stderr before `catch_unwind` ran. Fixed: `install_panic_hook_once` (guarded by `std::sync::Once`, called from `ffi_catch!`) routes panic messages through `tracing::debug!` instead, per `[F12]`'s existing logging-level policy — a no-op under `cfg(test)` so it doesn't silence other tests' panic diagnostics in the same test binary. 3 new tests.
...
- **Image→OCR flow — `[A7]` Architecture, informational for now.** ADR-009's `OcrEngine` callback interface has no `ParseLimits`-style size/dimension cap on the raw image bytes it passes across FFI, unlike every other importer in this plan. `Core::import_image_with_ocr` is still `todo!()` (M3 scope) so this isn't urgent yet, but the ADR-009 addendum defining that cap should be written before M3 OCR wiring begins, not discovered mid-implementation.
  - **Update 2026-09-24:** the concrete code-level gap this bullet warns about is now fixed (see §2.7's update note) — `gist-imageprep::prepare_image` checks declared image dimensions before decoding. The ADR-009 addendum itself and the OCR wiring remain open, M3 scope, unchanged from this bullet's original framing.
...
**Note (this section is otherwise stale relative to `CLAUDE.md` — kept minimal here per this pass's scope, not rewritten):** by 2026-09-12 the flow view was substantially built out past prototype quality (typography controls, progress + scroll-position persistence, keyboard/trackpad navigation, and a TOC now nested by heading level rather than a flat list — `FlowDocumentModel.swift`'s `TocEntry.indentLevel`, `FlowReaderContainer`'s `TocListView`), and the library search field (a different screen — §3.2 above, not this one) now focuses via ⌘F (`LibraryView`'s `.searchFocused`). `xcodebuild build`/`test` (scheme `GISTmacOS`) passes 43 real tests as of this pass. See `CLAUDE.md`'s M2 item 5 (and item 2 for the search shortcut) for the full detail this document doesn't carry item-by-item.
...
**Update 2026-09-24: `[A3]` ✅ Closed 2026-09-18, not open.** `apps/apple/macOS/GISTmacOS.entitlements` was added (app-sandbox, user-selected file read-write, network client only) and wired via `CODE_SIGN_ENTITLEMENTS` in `project.yml`. Decision and rationale recorded in `docs/adr/012-macos-app-sandbox.md`. Verified: an unsigned CI-style build plus 43/43 tests pass, and a separately ad-hoc-signed build confirms via `codesign -d --entitlements -` that all three entitlements are genuinely embedded, not just declared in source. **One residual caveat, unchanged since 2026-09-18:** no person has yet done an actual runtime click-through of a sandboxed launch (this dev environment has no Accessibility permission for `osascript`) — flagged in the ADR itself as the one remaining step. See `CLAUDE.md`'s `A3` register row for the identical closure language this note mirrors.
...
**Exit criterion:** a team member can use it as their daily reader, including searching their library and removing items they no longer want. Not met yet — no compiled build exists to try, and the library/search/removal/theme/flow-view gaps above are real blockers even once it compiles. `[F14]`–`[F23]` are all fixed in code (2026-09-12). `[F24]` remains genuinely open: its code-side prep (`.github/dependabot.yml`) is done, but the repo has no GitHub remote yet (§1.3), so there is still no live compensating control for the disabled `cargo deny check advisories` gate — don't treat M2 exit as closing this one.

**Update 2026-09-24:** `[F24]` is now closed (2026-09-19 — see §1.3's updated note above), so it no longer applies as an M2-exit caveat. This paragraph otherwise describes the state as of 2026-09-12 and is left as history; see `CLAUDE.md`'s M2 milestone-register row for the actual, much-later M2 exit status (feature-complete, security gate `F14`–`F24`/`N1`–`N7` all closed, only the manual click-through outstanding).
...
- **Security follow-ups (audit 2026-09-12):** `[F20]`/`[F21]` ✅ fixed in code the same day (see §2.4/§2.8) — no longer M3 work; `[F25]` ✅ closed 2026-09-12 — `apple-build` CI stood up for real and locally verified (see §1.3) — no longer M3 work either; `[A7]` ADR-009 addendum (OCR image-byte size/dimension cap) written **before** OCR pipeline wiring begins in this milestone, not after.

**Exit criterion:** RSVP frame timing verified stable at 1000 WPM; VoiceOver navigable end to end; entitlements file reviewed and approved. **Also gating exit:** `[A7]` addendum written. (`[F25]` closed 2026-09-12, ahead of this milestone — no longer gates M3 exit.)

**Update 2026-09-24:** the entitlements-review half of this exit criterion is met ahead of this milestone, same pattern as `[F25]` — `[A3]` ✅ closed 2026-09-18 (see §3.10's update note). `[A7]`'s addendum is still not written and the OCR wiring itself is still open, so `[A7]` continues to gate M3 exit as described — only its concrete code-level gap (`N4`) closed early, on 2026-09-18 (see §2.7).

**Update 2026-09-25:** the RSVP wall-clock drift fix and the annotations backend CRUD scaffold (schema v6, ADR-003 — anchoring/re-anchoring logic explicitly deferred) have landed since the note above; see `CLAUDE.md`'s M3 register row for the exact commits. The remaining M3 backlog (annotation anchoring, RSVP UI polish, annotation UI, settings, localisation, accessibility/TTS, and the OCR pipeline including `[A7]`'s still-open addendum) has been refined into a dependency-ordered set of agent roles in **`docs/m3-agent-roles.md`**, which is now the authoritative *execution* plan for M3 — this section remains the authoritative *scope and exit-criteria* reference, unchanged. That plan is triggered by the phrase "proceed with development", per `CLAUDE.md`'s Working conventions section.

**Update 2026-09-26:** that agent-role plan executed in full — every role in `docs/m3-agent-roles.md` §2 is done and merged onto `integration/m3-2026-09-26`, re-verified for real on the fully-integrated tree (`cargo test --workspace`/`clippy -D warnings`/`fmt --check`/`cargo deny check bans licenses sources` all green; `xcodegen generate` + `xcodebuild build`/`test` — BUILD SUCCEEDED, 163/163 tests, up from 49 at M2 exit). `[A7]` is now **fully closed** — both the ADR-009 addendum (documentation half) and `Core::import_image_with_ocr`'s real implementation (code half) landed; see `docs/adr/009-ocr-callback-interface.md`'s addenda and `CLAUDE.md`'s security register. Against this section's exit criterion: RSVP view polish landed with a documented, reasoned decision to keep wall-clock-state-driven redraw over `CVDisplayLink` (see `RsvpWallClockEngine`'s doc comment) rather than "verified stable at 1000 WPM" via live measurement, which this environment cannot do; a VoiceOver *code-inspection* pass landed (label/trait/hint fixes, contrast-ratio tests catching a real accent-color bug, Dynamic Type preview coverage) but **"VoiceOver navigable end to end" is not met** — no live screen-reader run has happened, same standing limitation as M2's manual click-through (no Accessibility permission for `osascript`/System Events here). Entitlements review (`[A3]`) was already closed 2026-09-18. **M3 is feature-complete but not exit-clean**: the one remaining gate is a person doing a real device pass (VoiceOver navigation, hearing read-aloud actually speak, eyeballing Dynamic Type/contrast on a real display, and the standard manual click-through) — tracked the same way M2's was, as a checklist for a dedicated session rather than re-derived each time.
...
- **Static-vs-dynamic FFI linking check, before first real signed Release build `[N8]` (new 2026-09-25):** a sibling Rust-core/native-shell app (contextGIST) shipped a real production incident with the identical shape as `gist-ffi`'s own `Cargo.toml` (`crate-type = ["staticlib", "cdylib", "rlib"]`): `cargo build` emits both a `.a` and a `.dylib`, and any link step using `-l<name>` against a search path containing both silently prefers the dylib. Under Debug (ad-hoc signing, Hardened Runtime off) this is invisible; the first real signed Release build with Hardened Runtime enabled rejected the separately-signed dylib at launch (`Library not loaded ... different Team IDs`). GIST's shipped path (`tools/build-core-xcframework.sh` → `GistCore.xcframework`, `project.yml` `embed: false`) already avoids this by selecting the `.a` by explicit full path rather than `-l`/search-path resolution, so the app links `gist-ffi` statically into one Mach-O with nothing else to sign/validate — but this has never been exercised under a *real signed Release* build, only unsigned CI-style ones. `tools/verify-ffi-panic-swift.sh` (a standalone, non-shipped test harness) does use the `-lgist_ffi` search-path pattern against a directory containing both artifacts — harmless today only because it's an unsigned, non-sandboxed CLI probe outside the app bundle. **Before the M4 notarised-DMG pipeline ships:** (1) do at least one real signed (not ad-hoc) Release build of `GISTmacOS.app` with Hardened Runtime enabled and confirm launch succeeds with no `Library not loaded`/library-validation errors; (2) keep `build-core-xcframework.sh`'s explicit-path `.a` selection — do not refactor it to `-l`/search-path linking; (3) if `gist-ffi`'s `crate-type` ever needs to drop down to just `["staticlib", "rlib"]` (cdylib appears unused by the shipped path — `uniffi-bindgen generate` in `build-core-xcframework.sh` already reads the `.a`), confirm `verify-ffi-panic-swift.sh` is updated in lockstep since it currently depends on the dylib.
...
- `[F22]`/`[F23]` ✅ fixed in code 2026-09-12 (see §2.11/§2.2) — no longer M4 work.
...
**Exit criterion:** a tag produces a DMG a stranger can download and open without a Gatekeeper warning; no known high or medium severity findings open in the security register. **Also gating exit:** `[A4]`/`[A6]` decisions recorded (ADRs written, implemented if warranted).

**Update 2026-09-24:** both decisions are recorded — ADR-011 (`[A6]`, encryption-at-rest) and ADR-013 (`[A4]`, at-rest integrity) — and both are Rust-side implemented as of 2026-09-18, satisfying the letter of this exit criterion early. Nuance: `[A6]` went further the same day via ADR-014 (per-item on-demand encryption) and is **switched on for production** — `CoreClient.shared` now opens via `newWithReadKey` with a real `KeychainKeyProvider`, so an item a user explicitly encrypts is genuinely readable afterward. New imports still land plaintext by default, unchanged. `[A4]`'s BLAKE3 integrity checksums remain Rust-side only, with no Swift/FFI/UI wiring yet (a `ChecksumMismatch` is an opaque, unpresented `GistError` today) — "implemented if warranted" is true for `[A4]` but not yet "switched on for production" in the same sense as `[A6]`. See `CLAUDE.md`'s `A4`/`A6` register rows for the full detail this note summarizes.
...
| **R7** | **Notarisation and Gatekeeper.** First-time notarisation with `libpdfium` reliably surfaces problems. | M | M | Throwaway end-to-end notarisation during M0. Prefer staticlib. Automate and run on every tag. Review entitlements explicitly in M3 `[A3]`. **Update 2026-09-24: `[A3]` done — ✅ closed 2026-09-18, see §3.10.** **Update 2026-09-25 `[N8]`:** "prefer staticlib" is followed for the shipped `gist-ffi` link path today, but has never been checked under a real *signed Release* build with Hardened Runtime on — a sibling app hit exactly this class of failure (dylib emitted alongside the staticlib from an identical `crate-type`, silently linked instead under `-l`/search-path resolution, then rejected by Hardened Runtime's library validation on first real signed Release build). See §5 M4 for the specific pre-ship check. |
...
| **R11** | **Scope: three UI codebases, small team.** | M | M | macOS-only for v1.0. Do not begin Windows until the iOS shell has validated the core API is genuinely platform-neutral. **Update 2026-09-24 (cross-reference, not a reversal of this row's original text):** this mitigation was deliberately not followed — Windows work began directly, without waiting for an iOS shell. `docs/windows-development-plan.md` records this explicitly as the "R11 exception" (its §3, discussion under ADR-018): W0/W1 serve as the platform-neutrality test in place of iOS. `PLATFORM_VERIFICATION.md` shows W1 (Rust core + `CoreClient` + DPAPI key custody) and W2 (Library screen, removal semantics, FlaUI click-through tests) verified 2026-09-20 through 2026-09-22, with ADR-015 (FFI binding), ADR-016 (key custody), ADR-017 (packaging/sandbox), and ADR-018 (UI stack) all recorded. Full detail lives in `docs/windows-development-plan.md`, which is the authoritative plan for that work — not duplicated in this macOS-focused document. |
...
| **R15** | ~~**No CI gate on the Swift half of the app.**~~ **Resolved 2026-09-12.** The documented `apple-build` pipeline didn't exist; every Swift change was hand-verified, not compiler-checked. Fixed: `.github/workflows/apple-build.yml` written, and `xcodebuild build`/`test` on the `GISTmacOS` scheme actually run and verified locally (this environment has full Xcode 26.6) — catching and fixing a real scheme/`TEST_HOST` bug along the way. | M | M | `[F25]` ✅ Closed. See §1.3. |
...
**Update 2026-09-24: both halves of Q11 are now decided, not open.** ADR-011 (`[A6]`, encryption-at-rest) and ADR-013 (`[A4]`, at-rest integrity via BLAKE3 sidecars) were both written and Rust-side implemented 2026-09-18. `[A6]` went further the same day (ADR-014, per-item on-demand encryption) and was switched on for production — new imports still default to plaintext, but an explicitly-"Encrypt"-ed item is genuinely readable through the running app via `CoreClient.shared`'s `KeychainKeyProvider`-backed read path. `[A4]`'s checksums are computed and verified on every read but have no Swift/FFI/UI presentation yet. See `CLAUDE.md`'s `A4`/`A6` register rows for full detail.
...
| A3 | Architecture | Open, historical row — **✅ Closed 2026-09-18, see update below** | M3 (entitlements review). **Update 2026-09-24:** `apps/apple/macOS/GISTmacOS.entitlements` added (app-sandbox, user-selected file read-write, network client only), wired via `CODE_SIGN_ENTITLEMENTS`; decision in `docs/adr/012-macos-app-sandbox.md`. Verified via an unsigned CI-style build (43/43 tests) and, separately, `codesign -d --entitlements -` on an ad-hoc-signed build confirming all three entitlements are genuinely embedded. Residual: no runtime click-through of a sandboxed launch by a person yet (no Accessibility permission for `osascript` in this dev environment). See `CLAUDE.md`'s `A3` row, and §3.10 above. |
...
| F20 | Low | ✅ Closed 2026-09-12 | (audit `L-2`). epub DRM check treated a malformed `EncryptionMethod` (no `Algorithm` attribute) as "no DRM." Fixed: rejects on ambiguity. See §2.4. |
| F21 | Low | ✅ Closed 2026-09-12 | (audit `L-3`). `gist-web` robots.txt lookup dropped the original URL's port. Fixed via `Url::join`-based `robots_url_for`. See §2.8. |
| F22 | Low | ✅ Closed 2026-09-12 | (audit `L-4`). `ffi_catch!` installed no custom panic hook. Fixed via `install_panic_hook_once`, routing through `tracing::debug!`. See §2.11/§5 M4. |
| F23 | Low | ✅ Closed 2026-09-12 | (audit `L-5`). `store_original_copy` didn't defensively re-validate `ext`. Fixed via a new `sanitize_ext` helper. See §2.2/§5 M4. |
| F24 | Low | Open, historical row — **✅ Closed 2026-09-19, see update below** | (audit `L-7`). No Dependabot config, and no compensating control for the disabled `cargo deny check advisories` gate. `.github/dependabot.yml` added, but this repo has no GitHub remote at all, so it has never run and provides no protection yet. Close only once pushed to GitHub with Dependabot alerts/security updates enabled there. See §1.3. **Update 2026-09-24:** both preconditions were met 2026-09-19 — the project is pushed to `https://github.com/BelongaGezza/gist`, and "Dependabot alerts"/"Dependabot security updates" were enabled via the GitHub API (verified with a follow-up read, not just requested). Dependabot proved itself live the same day via ~19 real update PRs. Mirrors `CLAUDE.md`'s `F24` row. |
| F25 | Informational | ✅ Closed 2026-09-12 | (audit `I-1`). Documented `apple-build` CI pipeline didn't exist; no CI gate on the Swift half of the app. Fixed: `.github/workflows/apple-build.yml` written; `xcodebuild build`/`test` (scheme `GISTmacOS`) actually run and verified locally, catching and fixing a real scheme/`TEST_HOST` bug (`GISTTests` not wired into the test action; `TEST_HOST` pointed at a nonexistent binary due to a `PRODUCT_NAME`/target-name mismatch), both fixed in `apps/apple/project.yml`. See §1.3/§6 R15. **Documentation-drift correction, 2026-09-12 (sixth pass):** this row, and four other places in this document, kept describing the finding as open after `CLAUDE.md` had already recorded it closed the same day — corrected in this pass. |
| A6 | Architecture | Open, historical row — **decided, implemented, and switched on for production 2026-09-18, see update below** | M4 decision (audit `I-3`). At-rest confidentiality undecided for plaintext IR blobs (ADR-007) and original-copy files (ADR-006) — broadens Q11, needs a new **ADR-011**. See §5 M4/§7 Q11. **Update 2026-09-24:** ADR-011 decided; Rust-side AES-256-GCM crypto + `KeyProvider`/schema v5 migration implemented and tested 2026-09-18; a real macOS Keychain round trip (create-once, 20-way concurrent-create race) independently verified the same day. **Later the same day, ADR-014 (per-item on-demand encryption) switched this on for production:** `CoreClient.shared`'s production `init()` now opens via `newWithReadKey` with a real `KeychainKeyProvider`, so an item a user explicitly runs "Encrypt" on is genuinely readable afterward through the running app — a real read-after-encrypt bug was caught by review and fixed before this shipped. New imports still land plaintext by default, unchanged. Mirrors `CLAUDE.md`'s `A6` row exactly; not reproduced further here. |
| A7 | Architecture | Open, historical row — **partially closed 2026-09-18, see update below** | Before M3 OCR wiring (audit `I-4`). ADR-009's OCR callback interface has no `ParseLimits`-style size/dimension cap on image bytes crossing FFI — needs an ADR-009 addendum. See §2.7/§3.3. **Update 2026-09-24:** the concrete code-level gap this item warns about (`gist-imageprep::prepare_image`'s decode-before-check ordering, independently found and tracked as `N4` in `docs/security-review-v2.md`) was fixed 2026-09-18 — it now peeks declared image dimensions and rejects an oversized image before the full decode. Still open: the ADR-009 addendum itself, and the OCR wiring (`Core::import_image_with_ocr` is still `todo!()`) — both remain M3 scope, unchanged. Mirrors `CLAUDE.md`'s `A7` row. |
...
**Open items blocking release:** F10 (M4), A3 (M3), A4 (M4), A6 (M4). F12 accepted and deferred. F14–F25 all fixed in code 2026-09-12 (see above). F24 remains genuinely open — no GitHub remote exists for this repo yet, so `.github/dependabot.yml` has never run. Additionally open per the 2026-09-12 audit: A7 (before M3 OCR wiring).

**Update 2026-09-24 (documentation-accuracy correction — this paragraph is now stale as a "current status" summary; kept as history, not rewritten):** as of this pass, the real state is: **F10** still open (M4, release pipeline is a deliberate stub). **A4** decided and Rust-side implemented 2026-09-18, not yet switched on for production (no Swift/UI wiring) — still meaningfully open. **A6** decided, Rust-side implemented, Keychain-verified, **and switched on for production** 2026-09-18 via ADR-014 for explicit per-item encryption (new imports still default to plaintext) — no longer blocking in the same sense as `A4`. **F12** still accepted/deferred, unchanged. **F14–F25** still fixed in code 2026-09-12, unchanged. **F24** is now ✅ closed (2026-09-19 — the repo is GitHub-hosted and Dependabot is verified enabled). **A3** is now ✅ closed (2026-09-18 — entitlements file added and verified embedded; one residual runtime-click-through caveat remains). **A7** is partially closed (code-level gap `N4` fixed 2026-09-18; the ADR-009 addendum and OCR wiring itself remain open, M3 gate). See the per-row updates above and `CLAUDE.md`'s security register for full detail.
