...
| F24 | Low | Open | (audit `L-7`). No Dependabot config, and no compensating control for the disabled `cargo deny check advisories` gate. `.github/dependabot.yml` added, but this repo has no GitHub remote at all, so it has never run and provides no protection yet. Close only once pushed to GitHub with Dependabot alerts/security updates enabled there. See §1.3. |
