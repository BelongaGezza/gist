...
- **Security follow-ups (audit 2026-09-12):** `[F20]`/`[F21]` ✅ fixed in code the same day (see §2.4/§2.8) — no longer M3 work; `[F25]` either stand up `apple-build` CI for real or stop claiming it exists; `[A7]` ADR-009 addendum (OCR image-byte size/dimension cap) written **before** OCR pipeline wiring begins in this milestone, not after.

**Exit criterion:** RSVP frame timing verified stable at 1000 WPM; VoiceOver navigable end to end; entitlements file reviewed and approved. **Also gating exit:** `[F25]` closed, `[A7]` addendum written.
...
**Open items blocking release:** F10 (M4), A3 (M3), A4 (M4), A6 (M4). F12 accepted and deferred. F14–F23 all fixed in code 2026-09-12 (see above). F24 remains genuinely open — no GitHub remote exists for this repo yet, so `.github/dependabot.yml` has never run. Additionally open per the 2026-09-12 audit: F25/A7 (before M3).
