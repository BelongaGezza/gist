...
**Last updated:** 2026-09-12 — M2 milestone section rewritten from a generic feature list to an accurate progress snapshot (import + RSVP wired; search/URL-import/removal/library/theme/flow-view not yet wired or not started); verified against the current codebase, not just prior notes.
...
├── fixtures/                   # ✅ 24 synthetic public-domain fixtures: txt/epub/docx/web
...
24 synthetically generated public-domain fixtures in `fixtures/`. Provenance recorded in `fixtures/README.md`. `tools/gen-fixtures` produces adversarial cases on demand (zip-bomb candidates, etc.).
...
- `delete_item(id)`: single-item deletion — **correction to this doc's own previous entry here, re-verified 2026-09-12.** An earlier pass claimed this doesn't cascade to `reading_progress`/`tokens`/`fts_index`; that was wrong — I missed that `Store::open` sets `PRAGMA foreign_keys = ON` and both `reading_progress.item_id` and `tokens.item_id` are declared `REFERENCES library_items(id) ON DELETE CASCADE`, so `DELETE FROM library_items` cascades to both tables automatically, and the `tokens_ad` trigger then clears the corresponding `fts_index` rows. A single statement is already atomic under SQLite, so no explicit `BEGIN`/`COMMIT` is needed for this to be safe. What's genuinely still missing, confirmed by reading the function: it deletes the `.json`/`.tokens.json` files *before* the DB row (best-effort, ignores errors) — the opposite order from the "delete source file only after the transaction commits" guarantee product-spec v1.5 §4 calls for, so a failed DB delete today can leave an orphaned library record pointing at already-deleted files. There are also no `annotations`/`bookmarks` tables yet (M3 features, not built), so there's nothing there to cascade regardless.
- **No `remove_items` (plural, multi-id) function exists anywhere in `gist-store` or `gist-core`.** Bulk removal — and the file-deletion-after-commit ordering guarantee — described in product-spec v1.5 §4 and macOS UI §3.2 is design intent, not shipped code. Single-item removal's data-integrity story is closer to done than a first read suggests, but the ordering bug above should be fixed as part of building `remove_items`, not left in place. See the M2 milestone status below.
...
- `GistCore` uniffi object with constructor `new(db_path, storage_dir)` plus exported methods: `health`, `import_txt`, `import_file`, `list_items(offset, limit)`, `search_items(query, limit)` (✅ added 2026-09-12, returns `Vec<FfiLibraryItem>`), `start_rsvp(item_id, wpm)`, `save_progress(item_id, token_index)`, `import_image_with_ocr(path, engine)`.
...
- On confirmation: calls `CoreClient.removeItems(ids:deleteSourceFiles:)`, which calls `gist-core::remove_items` — a single SQLite transaction covering metadata + annotations + FTS5 entries. Source file deletion follows transaction commit. On error, surfaces a specific error sheet; never partially removes.
...
- Library grid/list with sort (name / source type / date added / date last read / progress), collections/tags/smart views — `LibraryView.swift` is currently a flat unsorted `List` (99 lines), `SidebarView.swift` an 11-line placeholder. No collections/tags data model exists yet in `gist-model`/`gist-store` — this is schema work, not just UI.
...
- Item removal (single-item context menu + bulk multi-select, confirmation sheet, transactional deletion) — spec'd in product-spec v1.5 §4 and above in §3.2, not yet implemented in `gist-store`/`gist-core` or the UI.
- URL-paste import — `gist-web` is fully implemented (fetch, robots.txt, readability) but has no FFI export and no UI entry point. Second-cheapest unblock, same reasoning as search: Rust-only wiring.
