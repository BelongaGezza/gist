...
- **Full-text search has no path to the UI.** `gist-store::search_items` (FTS5 MATCH) is implemented and unit-tested at the storage layer but has zero callers outside its own crate — no `gist-core` wrapper, no FFI export. This is the cheapest unblock on this list: pure-Rust plumbing, verifiable with `cargo test`/`clippy` without needing a compiled Swift build. **Prioritise this first.**
