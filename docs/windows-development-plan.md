...
### W1 — Skeleton, CoreClient, CI (1.5–2 weeks) — status 2026-09-21: DONE (engineering); human look at the window and MSIX run outstanding
> **Closeout.** Merged: #40 skeleton, #42 tooling, #43 shell, #44 key custody, #45 core client, #41 `windows-build.yml`, #47 app wiring. Evidence: 118 xunit tests pass locally and on `windows-latest`; `windows-build` (`build` + `arm64-crosscompile`) green on real Actions; the app launches, lists a seeded item via UI Automation and shows the blocking corrupt-key page with the key file byte-identical (scripted, reproduced independently). **Not done / unverified:** no screenshot or human visual check (Mica, title bar, icons); Retry never clicked; error pages other than corrupt-key not exercised at runtime; MSIX not installed/run (Developer Mode off); ARM64 cross-compiles in CI but has no runtime test; clean-machine runtime (ADR-017) open. **Open maintainer decisions:** recovery-key export (before Encrypt ships in W2), Windows App SDK redistribution terms, ARM64 runtime-test capacity (Q7).
...
### W2 — Library (2–3 weeks) — status 2026-09-22: DONE (engineering); human visual pass and the deferred items below outstanding
> **Closeout.** Merged: #54 view-models, #56 Library page, #57 dialogs, #58 real dialog host, #55 Windows paths + detailed removal outcome + orphan sweep, #60 client adoption + warning bar, #61 orphaned-tags fix, #64 complete-delete semantics + shared-copy safety + dialog race, #52/#63 FlaUI project + 23 click-through tests, #51 rusqlite 0.40 with an old-database compatibility net, #59 stale-DLL guard, #62 apple-build path filter. Decisions recorded: no key recovery in v1 (ADR-016); **Remove is a complete delete** (ADR-006 addendum) — one button, deletes everything GIST holds for the item, never the user's original, and a content-addressed stored copy shared by two items survives until the last one is removed. Bugs found by W2 work and fixed: a database at a path over MAX_PATH could not be opened by SQLite (`sqlite_path`); the orphan sweep's keep-list could delete a referenced file on case-insensitive paths; Enter did not open a focused row; focus fell to the nav pane after Esc; a dialog requested just after another closed was silently dropped; orphaned tags stayed in the Filter menu. **Evidence:** `GIST.Core.Tests` 204 passing; 23 FlaUI click-through tests passing over 5 consecutive runs on an unlocked desktop (need an interactive session, skipped by default); Windows CI (`build`, `arm64-crosscompile`) and Apple's `apple-build` green on the merged tree. **Not verified / deferred:** no human has looked at the window (theme rendering, Mica, layout at widths, Narrator, high contrast, dialog look, native picker look; checklist: `docs/qa-manual-clickthrough-windows.md`); right-click context menu and double-click not automated; overflow menu at narrow widths not exercised; real https import and the DRM epub dialog not exercised in the UI; UI tests not run in CI (an interactive non-required job is a proposal only); MSIX not installed/run; ARM64 cross-compiles but has no runtime test; nothing run against a real macOS library (Apple items in `PENDING_APPLE_CHANGES.md`).
