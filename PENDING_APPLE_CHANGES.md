...
**Reason:** review finding Q10 (Windows W2). Removal deleted stored files best-effort and logged failures at `debug!`, so the UI could report "removed" while a stored copy was still on disk. `removeItemsDetailed` reports removed ids plus deleted/failed file counts and a coarse per-file failure kind (locked/permission/not-found/other — no paths or titles cross the boundary, per the source-path logging policy); `sweepOrphanedFiles` reclaims anything a failed delete left behind on a later launch. It matters most on Windows (a handle without `FILE_SHARE_DELETE` makes the delete fail outright), but macOS is not immune — an SMB/AFP share or a file with `uchg`/`schg` set fails the same way, and the sweep is the only thing that ever cleans those up.
...
**Verified where:** Windows 11 only — `cargo test --workspace` (25 `gist-core`, 46 `gist-store`), `cargo clippy --workspace -- -D warnings`, `cargo fmt --check`, plus the regenerated C# bindings building and `dotnet test` passing 118 tests. **Nothing was run on macOS/iOS**: the Swift bindings were not regenerated, no `xcodebuild` ran, and the new Rust code has never executed on an Apple platform. The macOS-only tests here are the `#[cfg(unix)]` symlink-sweep test (also runs on the Linux CI leg) and every unguarded test in the new group; the `#[cfg(windows)]` ones (delete-sharing, reserved names, junctions) do not compile there by design.
**Related commit/PR:** `feat/w2-windows-paths` (W2 Q10)
**Action:** on macOS run `./tools/gen-bindings.sh` + `./tools/build-core-xcframework.sh` and confirm `xcodebuild build`/`test` (scheme GISTmacOS) still pass with the larger generated surface. Optionally adopt: have `CoreClient.removeItems` call `removeItemsDetailed` and surface a non-zero `filesFailed` in the existing result-summary alert, and call `sweepOrphanedFiles()` once at launch. Delete this block once verified (adoption can be tracked separately if deferred).
